...
export const auditLog = pgTable(
  "audit_log",
  {
    ...
    actorSystemProcess: text("actor_system_process"),
    entityType: text("entity_type"),
    entityNameSnapshot: text("entity_name_snapshot"),
    entitySecondaryIdSnapshot: text("entity_secondary_id_snapshot"),
    impersonatorId: uuid("impersonator_id"),
    changes: jsonb("changes"),
    fieldChangesJsonb: jsonb("field_changes_jsonb"),
  ...
  (table) => [
    ...
    index("audit_log_entity_type_created_idx").on(table.entityType, table.createdAt),
    index("audit_log_visibility_scope_created_idx").on(table.visibilityScope, table.createdAt),
  ]
);
