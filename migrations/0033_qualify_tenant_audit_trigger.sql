-- Migration 0033: Make tenant audit trigger writes schema-qualified.
-- The original trigger function writes to unqualified `audit_log`, which can fail
-- in worker contexts even when tenant tables and search_path are otherwise present.
-- Using TG_TABLE_SCHEMA removes that runtime dependency.
