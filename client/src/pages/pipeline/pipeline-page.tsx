...
function DroppableColumn({
  ...
  const isCloseOut = slug === "close_out" || slug === "closeout";
  const isInProduction = slug === "in_production" || slug === "construction";
...
export function PipelinePage() {
  ...
  const wonStage = terminalStages.find((ts) => ts.stage.slug === "closed_won");
  const lostStage = terminalStages.find(
    (ts) => ts.stage.slug === "closed_lost" || ts.stage.slug === "lost"
  );
  const successRate = (() => {
    const won = wonStage?.count ?? 0;
    const lost = lostStage?.count ?? 0;
