...
// Wave 2 FilterBar (companies): Owner + Date(recency) + Sort. Search stays the page's own debounced,
// local-state input — the companies list has a HARD no-blank / no-URL-on-type requirement (#520,
// enforced by company-list-page.search-ux.test.tsx), which the bar's URL-backed `search` dimension would
// break. The mine/all owner scope stays a page control. The old Industry button row is removed (it was
// unused). Verification Status was dropped per product. Render order is fixed by the bar: Date, Owner, Sort.
// NOTE: the Date control's visible label is hardcoded "Date" in the shared bar; the AXIS is recency
// (COALESCE(last_activity_at, created_at)) — TODO(RED): a `dateLabel` prop so this reads "Last contacted".
const COMPANY_FILTER_DIMENSIONS: FilterDimension[] = ["rep", "date", "sort"];
...
export function CompanyListPage() {
  ...
  // A bar filter change (Owner/Date/Sort, or the page's mine/all scope writing the URL `scope`)
  // resets to page 1 — mirroring the search page control.
      ...
      <Card className="border-slate-200 bg-white shadow-none">
        <CardContent className="space-y-4 p-4">
          <div className="space-y-3">
            ...
            {/* Wave 2 shared FilterBar: Owner + Date (recency) + Sort. Search + mine/all scope stay page
                controls above (see COMPANY_FILTER_DIMENSIONS). */}
            <FilterBar
              ...
              // when false. The Date axis IS recency — COALESCE(last_activity_at, created_at) per contract.
              // TODO(RED): a per-surface date-note opt-out + a `dateLabel` ("Last contacted") so non-deal
              // mounts need not lean on this flag and the control reads as recency, not a generic "Date".
