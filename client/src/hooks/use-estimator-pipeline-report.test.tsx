...
const report: EstimatorPipelineReport = {
  ...
  scope: { kind: "active_office", cohort: "current_open_pipeline", note: "Current active office." },
  valueBasisLabel: "Best current estimate",
  pipeline: { count: 0, value: 0 },
  stageColumns: [],
  estimators: [],
  otherAssigned: { count: 0, value: 0, stages: [] },
  missingEstimator: { count: 0, value: 0, actionableCount: 0, actionableValue: 0, stages: [] },
  warnings: [],
};
...
describe("useEstimatorPipelineEvidence", () => {
  it("encodes the stable estimator key, exact stage slug, and pagination in the drill request", async () => {
    ...
    expect(mocks.api.mock.calls[0][0]).toBe(
      "/reports/estimator-pipeline/evidence?bucket=target&page=2&pageSize=25&estimatorKey=sidney_gibson&stageSlug=estimating",
    );
    expect(mocks.api.mock.calls[0][1].signal).toBeInstanceOf(AbortSignal);
  ...
  it("requests the missing-assignment bucket without an estimator or stage parameter", async () => {
    mocks.api.mockResolvedValue({
      data: {
        ...evidence,
        filter: { ...evidence.filter, bucket: "missing", estimatorKey: null, estimatorName: null, stageSlug: null, stageLabel: null },
      },
    });

    act(() => root.render(
      <EvidenceProbe request={{ bucket: "missing", page: 1, pageSize: 50 }} />,
    ));
    await flush();

    expect(mocks.api.mock.calls[0][0]).toBe(
      "/reports/estimator-pipeline/evidence?bucket=missing&page=1&pageSize=50",
    );
  });
