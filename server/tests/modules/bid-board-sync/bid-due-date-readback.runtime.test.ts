...
beforeAll(async () => {
  ...
  await pg.exec(`
    ...
    CREATE TABLE ${SCHEMA}.deals (
      ...
      -- Migration 0225: the provenance stamp the read resolver requires.
...
describe("Bid Board Due Date -> deals.bid_due_date (flag ON)", () => {
  ...
  // ★ PROVENANCE (migration 0225). The read resolver refuses the override without this stamp, so the write
