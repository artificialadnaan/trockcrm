...
beforeAll(async () => {
  ...
  await pg.exec(`
    ...
    -- buildContactLastTouchAtSql reads status + completed_at (a contact's last touch is work COMPLETED,
    -- not a task row's modification time), so this trimmed fixture needs both to match the real table.
    CREATE TABLE tasks (id uuid PRIMARY KEY DEFAULT gen_random_uuid(), contact_id uuid,
      status text NOT NULL DEFAULT 'pending', completed_at timestamptz, updated_at timestamptz);
  `);
