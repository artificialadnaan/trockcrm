...
describe("bid board mirror service", () => {
  ...
  it("writes won_closed_date from contract_signed_at on Won entry when only the timestamp is set (effective basis)", () => {
    const result = buildBidBoardMirrorUpdate({
      now: new Date("2026-04-23T18:00:00.000Z"),
      deal: {
        id: "deal-1",
    ...
    // Effective contract-signed = contract_signed_at::date wins over the stage-entry date.
    expect(result.updates.wonClosedDate).toBe("2026-02-15");
  });
