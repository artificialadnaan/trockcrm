...
async function seed(options: {
  ...
  /** Models writeBidDueDateIfNeeded's provenance stamp (0225). Absent => a coincidental day match. */
...
describe("Bid Board due date read precedence — flag ON", () => {
  ...
  const landed = {
    ...
    // The 0225 stamp. Without it the same two dates are only a COINCIDENCE and the override must refuse.
