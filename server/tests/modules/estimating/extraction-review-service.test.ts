...
describe("extraction-review-service", () => {
  ...
  it.each([
    ...
    ["a negative", "-5.000"],
    ["NaN", "NaN"],
  ])("REFUSES to approve a row whose quantity is %s, which would hide it", async (_label, quantity) => {
    // `needs_quantity` is the ONLY place an unpriceable row is visible: the worker reselects ordinary
    // extractions at `pending`, the workbench's bucket counts that status alone, and the promote
    // predicate refuses the stale recommendation. Approving wrote `approved` unconditionally, which
    // removed the row from the bucket while changing nothing about why it could not be priced —
    // leaving it approved, unpriceable, ineligible for re-pricing and invisible. Strictly worse than
    // before somebody tried to help it.
    const existingRow = {
      id: "ext-unpriceable",
      status: "needs_quantity",
      normalizedLabel: "Replace wall base",
      quantity,
      unit: "LF",
      divisionHint: "09",
    ...
    const tenantDb = {
      select: vi.fn(() => ({
        from: vi.fn(() => ({
          where: vi.fn(() => ({
            limit: vi.fn(() => ({ for: vi.fn().mockResolvedValue([existingRow]) })),
          })),
        })),
      })),
      update: vi.fn(() => ({ set: updateSet })),
    ...
    await expect(
      approveEstimateExtraction({
        tenantDb,
        dealId: "deal-1",
        extractionId: "ext-unpriceable",
        userId: "user-1",
      })
    ).rejects.toMatchObject({ statusCode: 400 });

    // Refused BEFORE the write, not rolled back after: nothing was sent.
    expect(updateSet).not.toHaveBeenCalled();
  });

  it("still approves a MEASUREMENT CANDIDATE with no quantity, which the worker prices anyway", async () => {
    // The exemption, and the reason the guard above is not a blanket rule. The worker's candidate
    // filter is `status = 'pending' OR extraction_type = 'measurement_candidate'`, so a measurement
    // candidate is priced on its TYPE regardless of status — it is not the stranded row the guard is
    // about, and refusing it would block a working path in order to protect a broken one.
    const existingRow = {
      id: "ext-measure",
      status: "pending",
      normalizedLabel: "Corridor area",
      quantity: null,
      unit: "SF",
      divisionHint: "09",
    ...
    const tenantDb = {
      ...
      update: vi.fn(() => ({
        set: vi.fn(() => ({
          where: vi.fn(() => ({
            returning: vi.fn().mockResolvedValue([{ id: "ext-measure", status: "approved" }]),
          })),
        })),
      })),
      insert: vi.fn(() => ({
        values: vi.fn(() => ({
          returning: vi.fn().mockResolvedValue([{ id: "evt", eventType: "approved" }]),
        })),
      })),
    } as any;
    ...
    await expect(
      approveEstimateExtraction({
        tenantDb,
        dealId: "deal-1",
        extractionId: "ext-measure",
        userId: "user-1",
      })
    ).resolves.toMatchObject({ extraction: { status: "approved" } });
  });
