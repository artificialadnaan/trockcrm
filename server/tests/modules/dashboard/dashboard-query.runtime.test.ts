...
beforeAll(async () => {
  await pg.exec(`
    ...
    -- scope_title is DISTINCT per row and one is NULL: the pair proves the value is carried per-deal
    -- from the column rather than defaulted, and the NULL proves the mapper does not invent a string.
    INSERT INTO deals (id, deal_number, name, is_change_order, scope_title, stage_id, assigned_rep_id, office_code, workflow_route, is_active, is_test_data, on_hold, bid_board_total_sales, bid_estimate, created_at, updated_at)
      VALUES ('${DEAL_CO_ID}', 'D-2', 'Tides Park Lane — Change Order 2', true, 'Panel Relocation', '${STAGE_ID}', NULL, NULL, 'normal', true, false, false, 100000, 90000, now(), now()),
             ('${DEAL_LOOKALIKE_ID}', 'D-3', 'Lobby — Change Order 1', false, NULL, '${STAGE_ID}', NULL, NULL, 'normal', true, false, false, 100000, 90000, now(), now());
...
describe("dashboard + pipeline SQL — real execution against a faithful schema", () => {
  ...
  it("getDashboardAtRiskRows carries deals.scope_title alongside the flag, per deal", async () => {
    // The SAME half-wired shape as the flag above, one column over. Once is_change_order relabels the
    // name to "<Parent> — Change Order N", scope_title is the only field left that says WHICH change
    // order — so a row that carries the flag and drops the title renders two siblings identically.
    //
    // Load-bearing in two directions: the CO row must carry its own distinct value (not a default),
    // and the lookalike must come back NULL (the mapper must not invent a string for a deal that has
    // none). A projection that never selected the column would fail the first; a mapper that
    // defaulted to "" or to the name would fail the second.
    const rows = await getDashboardAtRiskRows(testDb, {});
    const co = rows.find((r) => r.dealId === DEAL_CO_ID);
    const lookalike = rows.find((r) => r.dealId === DEAL_LOOKALIKE_ID);
    expect(co).toBeDefined();
    expect(lookalike).toBeDefined();
    expect(co?.dealScopeTitle).toBe("Panel Relocation");
    expect(lookalike?.dealScopeTitle).toBeNull();
  });
