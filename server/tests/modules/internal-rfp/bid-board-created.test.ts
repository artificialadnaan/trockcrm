...
describe("POST /api/internal/bid-board-created", () => {
  ...
  it("ACKs (200, not 422) a malformed 'created' for a terminally re-confirmed denial — don't make SyncHub retry an unprocessable callback", async () => {
    // terminal denial: reviewers upheld the no-go. A late malformed 'created' would no-op the linkage anyway
    // (the decision guard blocks it), so it must be acknowledged as an idempotent no-op rather than 422'd forever.
    queryMock.mockImplementation(async (sql: string) => {
      if (sql.includes("FROM pg_namespace")) return { rows: [{ nspname: "office_dallas" }] };
      if (sql.includes('FROM "office_dallas".deals') && sql.includes("LEFT JOIN")) {
        return {
      ...
      if (sql.includes('UPDATE "office_dallas".deals')) return { rows: [], rowCount: 0 }; // terminal-denial guard no-ops it
      return { rows: [], rowCount: 0 };
    });
    const { raw, signature } = sign({
      status: "created",
    ...
    // acknowledged (not 422) so SyncHub stops retrying; the decision guard ensures it never actually links/advances
    expect(res.status).toBe(200);
    expect(res.body).toMatchObject({ success: true, dealId: "deal-1" });
  });
});
