...
beforeAll(async () => {
  ...
  await pg.exec(`
    ...
    INSERT INTO field_scorecards VALUES
      ('${CARD_1}', 'card-1.v2.pdf', 'bucket', NOW(), 2),
      ('${CARD_2}', 'card-2.v2.pdf', 'bucket', NOW(), 2),
      ('${OTHER_CARD}', 'other.v2.pdf', 'bucket', NOW(), 2);
...
describe("scorecard PDF invalidation on evidence visibility changes", () => {
  ...
  it("invalidates linked snapshots when updateFile changes the evidence caption", async () => {
    ...
    const result = await pg.query<{ id: string; pdf_r2_key: string | null }>(
      "SELECT id, pdf_r2_key FROM field_scorecards ORDER BY id",
    );
