...
const ST_OPEN = u(902);
const ST_WON = u(901);
const ST_WON2 = u(904); // a different Won-family slug (closed_won)
const REP_A = u(1);
const REP_B = u(2);
...
beforeAll(async () => {
  ...
  await pg.exec(`
    ...
    INSERT INTO public.pipeline_stage_config (id, slug, name, is_terminal) VALUES
      ...
      ('${ST_WON2}', 'closed_won', 'Closed Won', true);
    INSERT INTO office_test.deals (id, name, deal_number, assigned_rep_id, stage_id, won_closed_date, expected_close_date, on_hold, is_active, is_test_data, awarded_amount, bid_estimate) VALUES
      ...
      ('${u(17)}', 'D7 ClosedWon','DFW-7','${REP_A}', '${ST_WON2}', DATE '2026-02-15', NULL,         false, true,  false, 150000, NULL);
  `);
...
describe("computeListDeals (PGlite, synthetic Dallas data)", () => {
  it("lists reportable deals (excludes on-hold/test/inactive), value DESC; reports total", async () => {
    ...
    expect(rows.map((r) => r.name)).toEqual(["D3 Tower", "D7 ClosedWon", "D1 Anthem", "D2 Haskell"]);
  ...
  it("stage 'won' matches the canonical Won family (slug 'won' AND 'closed_won')", async () => {
    const { rows } = await computeListDeals(db, { stage: "won" });
    expect(rows.map((r) => r.name)).toEqual(["D3 Tower", "D7 ClosedWon"]);
  });

  it("a non-'won' stage filter stays a literal slug/name fragment", async () => {
    const { rows } = await computeListDeals(db, { stage: "opportunity" });
    expect(rows.map((r) => r.name)).toEqual(["D1 Anthem", "D2 Haskell"]);
  });
  ...
  it("a real SQL total is reported even when the page is capped (truncated)", async () => {
    ...
    expect(totalMatching).toBe(4); // the count is the full match set, not the page size
