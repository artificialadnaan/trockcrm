...
function makeEngagement(id: string, type: "note" | "call" | "meeting" | "email", ownerId = "owner-1") {
  return {
    id,
    objectType: type,
    properties: {
...
function makeDeps() {
  ...
  return {
    ...
    findDealForHubspotEngagement: vi.fn(async () => ({
      status: "deal",
      deal: { id: "deal-1", name: "Jefferson on The Lake", hubspotDealId: "hubspot-deal-1" },
    })),
...
describe("hubspot-deal-activity-backfill script", () => {
  ...
  it("passes the validated office scope into HubSpot owner resolution", async () => {
    ...
    expect(deps.findUserForHubspotOwner).toHaveBeenCalledWith(
      {},
      "owner-1",
      expect.any(Map),
      "office-1"
  ...
  it("counts orphan, ambiguous, and unmapped user skips", async () => {
    ...
    deps.findDealForHubspotEngagement
      .mockResolvedValueOnce({ status: "orphan", hubspotDealIds: ["hubspot-deal-1"] })
      .mockResolvedValueOnce({ status: "ambiguous", hubspotDealIds: ["hubspot-deal-1", "hubspot-deal-2"] })
