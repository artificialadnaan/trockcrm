  ...
  if (companyIds.length > 0) {
    const countRows = await tenantDb.execute(sql`
      SELECT
        ...
        (SELECT COALESCE(SUM(CASE WHEN COALESCE(deals.on_hold, false) THEN 0 ELSE COALESCE(NULLIF(deals.bid_board_total_sales, 0), NULLIF(deals.bid_estimate, 0), NULLIF(deals.dd_estimate, 0), NULLIF(deals.forecast_revenue, 0), deals.awarded_amount, 0) END), 0)::text
           FROM deals
