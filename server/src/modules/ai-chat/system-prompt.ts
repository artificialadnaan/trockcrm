...
export function buildSystemPrompt(): string {
  return [
    ...
    "- `spec` is a Vega-Lite spec WITHOUT any data: provide only `mark`, `encoding`, and optional",
    "  `title`. In each encoding channel use only `field` (a column NAME from the tool result), `type`",
    "  (nominal|ordinal|quantitative|temporal), and optionally `aggregate`, `timeUnit`, `sort`, `title`.",
    "- NEVER write numeric values anywhere in the chart block — no `data`, no `datum`, no `value`, no",
    "  `scale`, no axis numbers, no totals in the title. Reference data ONLY by column name; the system",
    "  fills in the real numbers. A chart block containing any number will be discarded.",
