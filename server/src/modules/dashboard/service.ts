...
async function getDownstreamBottlenecks(
  ...
  return rows.map((row: any) => ({
    ...
    stageName: resolveMirroredStageLabel(row.mirrored_stage_slug, row.stage_name, row.workflow_route),
...
export async function getDirectorDashboard(
  ...
  return {
    ...
    downstreamBottlenecks,
    opportunityVsPipeline: {
      opportunityValue: ddResult.ddValue,
      opportunityCount: ddResult.ddCount,
      pipelineValue: ddResult.pipelineValue,
      pipelineCount: ddResult.pipelineCount,
      totalValue: ddResult.totalValue,
      totalCount: ddResult.totalCount,
    },
