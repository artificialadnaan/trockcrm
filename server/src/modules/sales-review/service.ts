  ...
  const newOpportunities = [
    ...scopedLeads
      ...
      .map((lead) => ({
        ...
        assignedRepName: lead.assignedRepId ? (userNameMap.get(lead.assignedRepId) ?? "Unknown User") : null,
  ...
  const hygiene = evaluateSalesHygieneRecords(
    [
      ...
      ...scopedDeals.map((deal) => ({
        ...
        ownershipSyncStatus: deal.ownershipSyncStatus,
        unassignedReasonCode: deal.unassignedReasonCode,
      })),
    ],
    { now }
  );
