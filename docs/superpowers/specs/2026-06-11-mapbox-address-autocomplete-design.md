...
- **Provider:** Mapbox. **Token:** a fresh `pk.*` stored **server-side** as `MAPBOX_TOKEN` env (Railway); the token shared earlier in chat is rotated/**dead** and must not be referenced. Because the token is used **server-side**, it must be **NON-URL-restricted**: a server `fetch` sends no `Referer`/`Origin`, so a URL-restricted token would be `403`'d and the feature would silently degrade. Its protection is server-side secrecy + the proxy's auth + the rate-limit, and it should be **scoped to the Geocoding API**. *Security model:* the token is **server-side only (never sent to the browser)**; its protection is env-var secrecy + the proxy's auth (CRM users only, via `authMiddleware` + `requireCrmUser`) + the endpoint rate-limit; it is at risk only from env compromise or an authenticated abuser, both bounded.
...
- **Server** (`server/tests/modules/address/service.test.ts`): unit-test the Mapbox-feature → `AddressSuggestion` parser (incl. partial `context`); degrade mapping for each trigger (no token / non-2xx / timeout → `{ suggestions: [] }`). Mock fetch — no live Mapbox.
- **Client** (`client/src/components/properties/address-autocomplete.test.tsx`): `<AddressAutocomplete>` — debounce fires one request; `<3` chars makes no request; select calls `onSelect` with parsed parts; error/empty → no dropdown AND next keystroke retries (no latch).
- **Invariant test** (`client/src/components/properties/property-selector.test.tsx`): in the repair-editor host, selecting a suggestion fills the 4 fields, the editor stays open, and the completeness gate still reports buildYear/unitCount missing (no "selected = complete").
