...
test, the value the canonical column held before the reopen deleted it* is a different act — the deal
enters no population, nothing is scored, and the value is not derived at all, it is `outcome_date`'s own
stored text out of the audit row that removed it. **There is no fallback, and this paragraph previously
