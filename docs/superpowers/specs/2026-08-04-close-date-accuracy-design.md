...
| 9 | The parse row and the alias-scope row each quoted a fence count | Codex read the parse row's `33` as stale against a document it counted as 31. It was not stale — 33 is right, and `31` is that row's own *inspected* figure — but **the miscount is the point**: a hand-quoted count of a growing artifact cannot be distinguished from a stale one by the reader it is written for, and this round had already corrected the alias-scope row's copy of the same mistake. Both rows now describe what the run **emits** and quote nothing. Measurements pinned to a named commit stay, because a commit does not change. |
