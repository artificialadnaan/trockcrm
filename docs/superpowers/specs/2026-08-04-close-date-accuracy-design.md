...
| **No quoted fence counts** | no **prose** line in this document quotes a fence, unit or inspection count | **extraction** — governed set is the whole document outside ```fences, which is the rule's own property. A numeral or number-word quantifying `fence`/`unit`, or adjacent to `governed`/`reached`/`inspected`/`eligible`, is a hit; commit hashes, `§` refs, round numbers, years and `fence@NNNN` locators are scrubbed first | mechanical *(added this round)* · **it exists because fixing the sites did not work three times.** The alias-scope row's tally went stale in three consecutive rounds; each round corrected the number, and round 29 then added a *new* row quoting a fresh one, which was stale within the same round because the round added a fence. Sites were being fixed where the mechanism was the defect. Every such figure is now an output of a run and appears nowhere in the text — including the historical ones in the appendix, which were rewritten to describe the shape without the number, since a commit-pinned measurement reads identically to a live one. **Known-positive, and it fired on this row's own first draft:** splice a numeral in front of the word `fences` anywhere in prose and the line is reported — which is what happened when this cell tried to quote an example of the thing it forbids. **Does not cover:** counts of anything else — metrics, populations, conventions, audit rows — which have gone stale here too (§8's "38 metrics", the audit tally's "eighteen"); this rule is deliberately narrow to the figure that recurred, and widening it is a judgement about which counts are derived rather than a mechanical one |
