...
| `lost_landing_date` (§4.0.5) | `count(*)` over `audit_log` rows on `deals` whose `changes` carry `won_closed_date` or `lost_at` with a **non-null `old`** and a null `new`, and of those, how many have a `deal_stage_history` reopen row at the **same `created_at`** | **Alarm on the first, and the diagnostic is inert:** no reopen in this office ever cleared a canonical outcome date, so `reopened_after_landing_n` will always read 0 and the period test has nothing to test. Non-zero on the first and zero on the second means the **pairing** is wrong — the clearing and the stage row are not landing in one transaction — and that is a bug in this predicate, not a property of the data |
