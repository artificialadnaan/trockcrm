...
- At today's 148×156.5pt tile, a 0.46:1 photo renders **width-bound**: 148 × 322pt — well under half the tile's height, with the tile itself already small relative to the page.

### Why "just reduce rows per page" is not sufficient on its own

`PHOTO_TILE_WIDTH` (148) and `PHOTO_TILE_HEIGHT` (derived from row count) are independent constants. Reducing `PHOTO_ROWS_PER_PAGE` alone grows tile *height* only. At the current 148pt width, a 0.46:1 photo is already **width-bound** — it renders at 148×322pt regardless of how much taller the tile gets, because contain-fit stops scaling once the narrower dimension is satisfied. Going from 4 rows to 1 row (156.5pt → 668pt of available tile height) would leave that same 148×322pt photo sitting in a much taller box with **more empty grey space below it, not a bigger photo** — the rendered image would not change size at all. Only widening the tile moves the number that's actually too small.
