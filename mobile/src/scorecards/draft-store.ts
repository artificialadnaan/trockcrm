...
/**
 * Rebase a draft's evidence photo uris onto the LIVE per-draft directory. copyPhotoIntoDraft freezes an
 ...
 * again. Any non-durable uri is left untouched. Pure-ish: no FS I/O, so read paths stay lock-free.
 */
function rebaseDraftPhotoUris(ownerKey: string, draft: ScorecardDraft): ScorecardDraft {
  ...
  const photos = draft.photos.map((photo) => {
    // Durable evidence is named `<photoDir>/<clientUploadId><ext>`; without a clientUploadId there is nothing
    // to rebuild from. (Scorecard photos are all durable per-draft copies — no retained/remote evidence.)
    if (!photo.clientUploadId) return photo;
...
export async function listScorecardDrafts(ownerKey: string): Promise<ScorecardDraft[]> {
  ...
  // Rebase every returned draft's photo uris onto the live document directory so a rotated iOS container path
  // self-heals on resume (covers BOTH the .tmp recovery path and the live index; loadScorecardDraft delegates
  // here). This fixes resumed drafts whose photos would otherwise render blank AND brick submit's copyAsync.
