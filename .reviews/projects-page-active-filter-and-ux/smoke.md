...
| `5ccb15f4-620b-4dad-9fd5-3846c2de3a88` | **SUCCESS** @ 20:55 | PR #256 hotfix(deploy) — landed all 4 PRs to prod. **This is the deploy serving the new code.** |

## Hotfix root cause

The runtime image stage runs `npm ci --omit=dev --workspaces`, which excludes the `husky` devDependency but still executes the `prepare` lifecycle script. PR #256 changed `package.json` `"prepare"` from `"husky"` to `"husky || true"` so the script exits 0 whether or not the binary is installed.
