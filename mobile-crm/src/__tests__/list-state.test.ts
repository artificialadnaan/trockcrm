...
describe("resolveListState", () => {
  ...
  it("does NOT block when a load succeeded and a later fetch failed", () => {
    ...
    const state = resolveListState({ ...base, data: { pages: [] }, error: new Error("5xx") });
  ...
  it("treats a loaded EMPTY result as loaded, not as never-loaded", () => {
    ...
    const state = resolveListState({ ...base, data: { pages: [] }, error: new Error("5xx") });
    expect(state).toEqual({ kind: "loaded", refreshFailed: true, pageFailed: false });
  });
  ...
  it("separates a load-more failure from a refresh failure", () => {
    ...
    expect(page).toEqual({ kind: "loaded", refreshFailed: false, pageFailed: true });
    ...
    expect(refresh).toEqual({ kind: "loaded", refreshFailed: true, pageFailed: false });
  });

  it("reports neither failure on a clean loaded list", () => {
    expect(resolveListState({ ...base, data: { pages: [] } })).toEqual({
      kind: "loaded",
      refreshFailed: false,
      pageFailed: false,
    });
  ...
  it("treats an empty ARRAY as loaded — the distinction is undefined, not falsiness", () => {
    ...
    const state = resolveListState({ ...base, data: [], error: new Error("5xx") });
    expect(state.kind).toBe("loaded");
  });
});
