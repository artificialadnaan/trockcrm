 ...
 *
 * WHY THIS EXISTS IN A NEW APP WITH NO IMAGES YET. It is ported from T-Rock Cam (mobile/), which only
 * got it AFTER shipping the crash — five core <Image> renders survived a migration that every reviewer
 * and every green check had already signed off on (#956). Starting mobile-crm with the invariant costs
 * nothing today and means this app can never acquire that bug. `Animated.Image` is covered too, since
 * that is the form the crash originally took.
 *
...
describe("no core react-native <Image> anywhere", () => {
  ...
  it("scans the whole mobile source tree", () => {
    ...
    // Sentinels for THIS app, not T-Rock Cam's. Low but non-zero: mobile-crm is new, so the count grows
    // with every feature PR — the point is only that the walker found a real tree rather than nothing.
    expect(files.length).toBeGreaterThan(8);
    expect(files).toContain("app/login.tsx");
    expect(files).toContain("src/auth/AuthContext.tsx");
  });
  ...
  it("would catch a core <Image> reintroduced into any real file's react-native import", () => {
    ...
    // Every screen imports from react-native, so this tracks the screen count. Deliberately a floor and
    // not an exact number, so adding screens never edits this line.
