...
6. **Goals system is OUT OF SCOPE for A5a**. `rep_performance_snapshots` ships without a goals table. Hook returns `forecast_vs_goal` with `goal: null` and `goal_source: 'none'` until a future scope (post-rollout, possibly Track F or a dedicated goals PR) designs the goals system properly. Track F's director dashboard must render a "no goal set" empty state when `goal_source === 'none'`.
