...
### `emails` (`emails.ts`)
Existing: `id`, `subject`, `from_address`, `to_addresses[]`, `cc_addresses[]`, `bcc_addresses[]`, `body_text`, `body_html`, `received_at`, `graph_message_id`, `graph_thread_id`, `assigned_entity_type`, `assigned_entity_id` (single FK), `assignment_confidence`, `assignment_ambiguity_reason`, attachments JSONB. **Tier 4 add**: `ai_suggestions` JSONB (array `{type, id, name, confidence}`). New `email_links` junction (`email_id`, `entity_type`, `entity_id`) OR `linked_records` JSONB.
