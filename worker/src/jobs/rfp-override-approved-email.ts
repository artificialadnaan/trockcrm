...
interface RfpOverrideApprovedPayload {
  ...
  // finding: the VOTING-path override-approve carries no SyncHub request id (the internal-rfp callback enqueues
  // this job app-side because the 0155 trigger skips a NULL request id). It passes the round event id as the
  // idempotency dimension instead.
...
export async function handleRfpOverrideApprovedEmail(
  ...
  // finding: the VOTING-path override-approve has no request id; it's keyed on the round event id instead. Accept
  // EITHER a request id (the legacy trigger-enqueued path) OR a round id (the app-enqueued voting path).
  ...
  if (!isSafeTenantSchema(tenantSchema) || !dealId || (rfpApprovalRequestId == null && rfpVoteRoundId == null)) {
    logger.warn("[RfpOverrideApprovedEmail] Invalid job payload - skipping", { tenantSchema, dealId, rfpApprovalRequestId, rfpVoteRoundId });
    return;
  }

  const query = deps.query ?? pool.query.bind(pool);

  // Exactly-once guard: skip if this RFP cycle's approval already sent. The receipts ledger is keyed on the
  // request id, so it only applies to the request-BACKED path. The request-less (voting) path is instead made
  // once-only by its enqueue site (the internal-rfp callback enqueues ONLY on the real declined->approved
  // transition, which fires once per round) + the round-scoped Resend idempotencyKey below.
  ...
  try {
    ...
    // Idempotency dimension: the request id when present, else the round event id (voting path). Both are stable
    // per RFP cycle, so a duplicate job (e.g. a retried worker tick) can't send twice.
    ...
    const sendResult = await sendEmail(recipients, email.subject, email.html, {
      text: email.text,
      idempotencyKey: `rfp-override-approved-${tenantSchema}-${dealId}-${idDimension}`,
    });
