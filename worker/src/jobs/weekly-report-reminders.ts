...
// Reuse the branded-email primitives (shell, detail table, frontend URL) so these reminders point at
 ...
 *
 * `appUrl` IS NULL UNLESS `WEEKLY_REPORT_APP_DEEP_LINKS=true`. The flag exists because a `trockcam://`
 * link the app does not recognise is not merely useless, it is DESTRUCTIVE:
 * `mobile/src/wearables/pairing-callback.ts` decides what to hold with a DENY-list — any URL whose route
 * key is NOT in `APP_OWN_ROUTES` counts as a possible Meta pairing callback and is retained, evicting a
 ...
 * `mobile/app/(app)/reports/weekly/[draftId].tsx`, whose segment is a LOCAL DRAFT id — generated by
 * `newClientUploadId()` and read back through `loadWeeklyReportDraft` — not a `weeklyReportProjectId`. A
 * server id in that slot matches no draft on the device, so the screen renders "Draft not found. It may
 * have been submitted or discarded." to a super whose report is neither, and `?weekOf=` is never read at
 * all. Opening a week the SERVER holds is an async decision (`mobile/src/weekly-reports/hub.ts`:
 * reconcile vs resume-local vs create-local) that only the hub makes, so the hub is the deepest
 * destination that actually resolves today — it lists the recipient's outstanding weeks and opens the
 * right one. The email body already names the project and the week. Route by week again in the same
 * change that gives the app a route accepting a project id and a week, not before.
 *
...
export function weeklyReportReminderLinks(input: {
  ...
  return {
    // No project id, no weekOf: the hub reads neither, and this function took both only to build a path
    // the app cannot resolve. Dead inputs are the same promise-to-the-reader as dead query parameters.
    appUrl: input.appDeepLinksEnabled ? "trockcam://reports" : null,
...
export interface WeeklyReportReminderEmailInput {
  ...
  weekOf: string;
  /** null unless `WEEKLY_REPORT_APP_DEEP_LINKS=true` — see weeklyReportReminderLinks. */
  appUrl: string | null;
  webUrl: string;
}
